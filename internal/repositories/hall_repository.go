...
func (r *hallRepository) GetAllHallBansPaginated(ctx context.Context, db database.DBRunner, hallID uuid.UUID, offset int, limit int) ([]*models.HallBan, error)

// check operation
func (r *hallRepository) IsUserBanned(ctx context.Context, db database.DBRunner, hallID uuid.UUID, userID uuid.UUID) (bool, error) {
	query := `	SELECT EXISTS (
					SELECT 1
					FROM hall_bans
...
func (r *hallRepository) GetBanByUserAndHall(ctx context.Context, db database.DBRunner, hallID uuid.UUID, userID uuid.UUID) (*models.HallBan, error) {
	query := `
				SELECT id, reason, user_id, hall_id, created_at, updated_at
				FROM hall_bans
				WHERE hall_id = $1 AND user_id = $2
...
// Statistics
func (r *hallRepository) GetBanCount(ctx context.Context, db database.DBRunner, hallID uuid.UUID) (int, error) {
